...
const SYSTEM_PROMPT = "Eres un experto en documentacion de codigo";

type Message = { role: 'user' | 'assistant' | 'system'; content: string };

class CloeAgent implements Agent {
  ...
  async initialize(_params: InitializeRequest): Promise<InitializeResponse> {
    console.error('[cloe] inicializado');
    return {
  ...
  async newSession(_params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    console.error(`[cloe] nueva sesion: ${sessionId}`);
    return { sessionId };
  }
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    console.error(`[cloe] prompt: ${userText.substring(0, 60)}`);
    ...
    console.error(`[cloe] respuesta enviada`);
...
if (process.stdin.isTTY) {
  ...
  console.log('\n[cloe] Modo interactivo. Escribe tu mensaje (Ctrl+C para salir):\n');
  ...
  new AgentSideConnection((conn) => new CloeAgent(conn), stream);

  console.error('\n[cloe] Agente ACP listo. Esperando conexion via stdin/stdout...');
}
